...
| 角色分配     | 接入已有角色选择与用户角色替换接口；不提供创建角色入口                                                                                      | 已确认；用户要求继续第二阶段 |
...
> 状态：部分完成；角色 API、分配 UI 和本地质量门禁已完成，真实测试环境写操作待受控联调
>
> 阶段计划：[ADMIN_USERS_IMPLEMENTATION_PHASE_2_PLAN.md](./ADMIN_USERS_IMPLEMENTATION_PHASE_2_PLAN.md)
>
> 执行记录：[ADMIN_USERS_IMPLEMENTATION_PHASE_2_EXECUTION.md](./ADMIN_USERS_IMPLEMENTATION_PHASE_2_EXECUTION.md)

前置依赖：

- 用户已确认角色分配纳入第二阶段；
- 角色列表接口 `GET /admin/roles` 和用户角色接口可用，并由环境管理员确认权限。
...
| ----------------------------- | -------------------------------- | --------------------------------- | ---------------------------- |
| 角色分配是否纳入本期          | 决定是否接入角色列表和 PUT roles | 纳入第二阶段                      | 已确认；用户继续执行第二阶段 |
